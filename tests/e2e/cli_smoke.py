...
check("config get RUN_MODE", rc == 0 and o.strip() in ("off", "paused", "headroom"), o)
...
# Global flags work BOTH before and after the subcommand (a leading --url once
# regressed to the default URL via a shared-action argparse quirk — keep both
# orders locked, including on a nested subcommand).
rc, o = run(["status", "--url", BASE, "--json"])
check("trailing --url/--json on a subcommand", rc == 0 and o.strip().startswith("{"), o)
rc, o = run(["--url", BASE, "cache", "status"])
check("leading --url reaches the right service (nested subcommand)", rc == 0, o)

# scoring get prints ONLY the scoring/filter keys.
rc, o = run(B + ["scoring", "get"])
check("scoring get is scoped to score keys",
      rc == 0 and "SCORE_BALANCE" in o and "HEADROOM_GB" not in o, o)

# dirs browses the library root.
rc, o = run(B + ["dirs"])
check("dirs lists library folders", rc == 0 and "movies" in o, o)

# notify test without destinations: clean refusal, not a crash.
rc, o = run(B + ["notify", "test"])
check("notify test w/o destinations refuses cleanly", rc == 1)
