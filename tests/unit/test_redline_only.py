...
check("file validator: headroom off with a cap and NO redline is valid",
      not A._config_file_issues(dict(_DIRS, HEADROOM_GB=0, REDLINE_GB=None,
                                     REDLINE_ONLY_MODE=True, MAX_LIBRARY_GB=5000)))
check("file validator: every threshold off at once is a VALID saved state",
      not A._config_file_issues(dict(_DIRS, HEADROOM_GB=0, REDLINE_GB=None,
...
check("file validator: no redline ceiling under an unarmed headroom, flag or not",
      not A._config_file_issues(dict(_DIRS, HEADROOM_GB=0, REDLINE_GB=900, MAX_LIBRARY_GB=None))
...
# ── Plan gate: Simulate is ALWAYS required before a cleanup in the mode ───────────
...
try:
    ...
    check("mode: Cleanup ghosts pending the preview plan", lb["cleanup_disabled"] is True)
    ...
    check("normal: satisfied keeps Simulate enabled, ghosts Cleanup",
...
with tempfile.TemporaryDirectory() as td:
    ...
    check("mode: forecast schedules nothing",
          ...
          and f["event_count"] == 0 and f["marked"] == 0)
...
# ── No-thresholds state (headroom 0, no redline): valid but cleanup-blocked ─────
...
check("no-thresholds state: Cleanup blocked with the setup message",
