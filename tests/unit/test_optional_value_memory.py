...
# (the shipped default) — it saves fine; Cleanup just stays blocked until a
# target exists.
...
# Headroom 0 IS the trigger off, whatever the posted flag says (it is derived
# from the value on save) — so a Redline alongside it is redline-only, with no
# ceiling to sit under, and the save stores a consistent flag.
code, body, cfg = save_cfg(BASE_SAVED, base_payload(HEADROOM_GB=0, REDLINE_GB=200))
check("headroom 0 with redline saves; the flag is derived, not demanded",
      code == 200 and cfg.get("REDLINE_ONLY_MODE") is True)
code, body, cfg = save_cfg(BASE_SAVED, base_payload(HEADROOM_GB=0, REDLINE_GB=200,
...
code, body, cfg = save_cfg(BASE_SAVED, base_payload(HEADROOM_GB=0, REDLINE_GB=None,
                                                    REDLINE_ONLY_MODE=True,
                                                    MONITOR_DIRS=["/library/movies"]))
check("every threshold off at once saves (the scheduler rests at Paused instead)",
      code == 200)
