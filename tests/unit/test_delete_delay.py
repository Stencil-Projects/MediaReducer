...
with tempfile.TemporaryDirectory() as td:
    ...
    # UNSTAMPED marks fall back to the current setting, so shortening the
    # delay still moves them up.
    _state["cfg"]["DELETE_DELAY_DAYS"] = 2
    entries = {e["title"]: e for e in A.pending_deletion_entries()}
    check("unstamped mark follows the current setting",
          entries["Movie A"]["days_remaining"] == 1)
    # A STAMPED mark keeps the delay it was made under — changing the setting
    # never re-dates it; only a re-mark or the delay-reset button does.
    _dbstate.seed(Path(td, "mediareducer.db"), {"pending": {"schema": 1, "entries": {
        "/library/movies/D/D.mkv": {"title": "Movie D", "size_bytes": 1_000,
                                    "score": 1.0, "marked_at": now, "delay_days": 7},
        "/library/movies/E/E.mkv": {"title": "Movie E", "size_bytes": 1_000,
                                    "score": 1.0, "marked_at": now, "delay_days": 2},
    }}})
    entries = {e["title"]: e for e in A.pending_deletion_entries()}
    check("stamped marks keep their own delay under a changed setting",
          entries["Movie D"]["days_remaining"] == 7
          and entries["Movie E"]["days_remaining"] == 2)
...
with tempfile.TemporaryDirectory() as td:
    ...
    stamp = {k: A._plan_config_value(_state["cfg"], k) for k in A._PLAN_CONFIG_KEYS}
    ...
    stamp_raw = {k: A._plan_config_value(_raw_file["value"], k)
                 for k in A._PLAN_CONFIG_KEYS}
...
# ── Arming Automatic Cleanup over breached limits requires a CURRENT Simulate plan ────────
# Healthy connections: an unhealthy probe force-pauses Automatic Cleanup before the gate.
A._refresh_connection_health_cache = lambda cfg, probe=True: {
...
check("delay 0 over limits also requires a plan (manual Cleanup deletes now)", code == 400)
...
code, cfg = save({"RUN_MODE": "headroom", "DELETE_DELAY_DAYS": 7})
check("over limits the snapshot alone does not arm Automatic Cleanup", code == 400)
...
with tempfile.TemporaryDirectory() as td:
    ...
    body = r.get_json()
    check("within-limits threshold change while armed keeps Automatic Cleanup armed",
    ...
    r = client.post("/api/config", json=dict(p0, HEADROOM_GB=400), headers={"X-MediaReducer": "1"})
    body = r.get_json()
    # This fixture arms Automatic Cleanup with NO server and NO monitored path
    # — reachable only through the raw API. The forced pause still fires with
    # its reason, but a working set with structural pieces missing now rests
    # all the way at Paused ("off") instead of parking at Monitor Only; the
    # Monitor-Only landing for an intact set is pinned by the within-limits
    # case above and by test_reset_then_setup.
    check("over-limit threshold change while armed force-pauses",
          r.status_code == 200 and _state["cfg"].get("RUN_MODE") == "off"
    ...
    # ARMING a new threshold that itself creates the breach (a cap below the
    # library) pauses the same way — the breach signal gates, whichever
    # threshold caused it.
    _state["cfg"] = dict(_state["cfg"], RUN_MODE="headroom", HEADROOM_GB=500, OUTPUT_DIR=td)
    r = client.post("/api/config", json=dict(p0, MAX_LIBRARY_GB=50), headers={"X-MediaReducer": "1"})
    check("arming a cap that pushes it over a limit force-pauses",
          r.status_code == 200 and _state["cfg"].get("RUN_MODE") == "off")
    _state["cfg"]["MAX_LIBRARY_GB"] = None   # restore: the next case is NON-threshold
    ...
    body = r.get_json()
    check("unrelated save while armed keeps Automatic Cleanup armed",
          r.status_code == 200 and _state["cfg"].get("RUN_MODE") == "headroom"
          and not body.get("automatic_run_mode_paused"))
