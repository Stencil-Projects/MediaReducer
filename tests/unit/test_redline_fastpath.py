...
try:
    with tempfile.TemporaryDirectory() as td:
        ...
        engine.write_plan_to_queue([(cand, 2 * MB)])    # scheduled_count 0
        ...
        engine.write_plan_to_queue([(cand, 2 * MB)], scheduled_count=1)
        e = _pending()["entries"][str(paths[0])]
        ...
        engine.write_plan_to_queue([(cand, 2 * MB)])    # left the prefix
        e = _pending()["entries"][str(paths[0])]
...
try:
    with tempfile.TemporaryDirectory() as td:
        ...
        engine.write_plan_to_queue([(cand, 2 * MB)])
        e = _pending()["entries"][str(paths[0])]
...
kept, new_marks, dropped, new_items = E.write_plan_to_queue(planned, scheduled_count=3)

check("all three kept", set(Path(k).name for k in kept) == {"A.mkv", "B.mkv", "C.mkv"})
check("one new mark (C)", new_marks == 1)
check("new-mark items name the new mark only",
      [it["title"] for it in new_items] == ["C"] and "delete_on" in new_items[0])
