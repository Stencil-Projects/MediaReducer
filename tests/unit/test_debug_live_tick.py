...
with tempfile.TemporaryDirectory() as td:
    ...
    count, freed, covers, _items = engine._debug_cleanup_delete_preview(5 * MB)
    check("preview reports the covering prefix (3 of the 2 MB files, target 5 MB)",
          count == 3 and freed == 6 * MB and covers is True)
    check("preview returns the would-delete items (title+size) matching the count",
          len(_items) == count and all("title" in it and "size" in it for it in _items)
          and sum(it["size"] for it in _items) == freed)
...
with tempfile.TemporaryDirectory() as td:
    paths = setup(td)
    count, freed, covers, _items = engine._debug_cleanup_delete_preview(50 * MB)
...
with tempfile.TemporaryDirectory() as td:
    ...
    count, freed, covers, _items = engine._debug_cleanup_delete_preview(2 * MB)
