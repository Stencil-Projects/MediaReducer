...
SONARR_SERIES = [{"id": 5, "title": "Dead Show",
...
CALLS.clear()
r = A._run_tv_cleanup_pass(CFG, execute=True)
check("a due, still-taken season is deleted",
      len(r["deleted_seasons"]) == 1 and r["deleted_files"] == 2
      and r["freed_bytes"] == 3000, r)
...
# ── The tick gate ───────────────────────────────────────────────────────────
_orig_pass = A._run_tv_cleanup_pass
ran = []
A._run_tv_cleanup_pass = lambda cfg, *, execute: ran.append(execute) or {}
reset_state()
A._maybe_run_tv_cleanup(dict(CFG, RUN_MODE="off"))
check("a fully-paused scheduler never runs the TV pass", ran == [], ran)
A._maybe_run_tv_cleanup(dict(CFG, MAX_LIBRARY_GB=None))
check("no pool trigger (no cap, no headroom) → no pass", ran == [], ran)
A._maybe_run_tv_cleanup(dict(CFG, USE_JELLYFIN=False))
check("no media server to supply the inventory → no pass (Sonarr alone is "
      "NOT enough — it is cleanup-only)", ran == [], ran)
A._maybe_run_tv_cleanup(dict(CFG, TV_CLEANUP_ENABLED=False))
check("the TV switch off → no pass", ran == [], ran)
A._maybe_run_tv_cleanup(dict(CFG, RUN_MODE="paused"))
check("Monitor Only runs the pass without the deleting half", ran == [False], ran)
A._maybe_run_tv_cleanup(CFG)
check("Automatic Cleanup runs it deleting", ran == [False, True], ran)
A._save_tv_cleanup_state({"marked": {}, "last_pass_date": time.strftime("%Y-%m-%d")})
A._maybe_run_tv_cleanup(CFG)
check("one pass per day — today's latch skips the second", ran == [False, True], ran)
A._run_tv_cleanup_pass = _orig_pass
