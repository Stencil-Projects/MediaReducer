"""Startup store preservation + the "full scan within two days" policy.
...
be trusted is decided LIVE by the space-threshold gate:
  ...
  • a full library scan must have completed within the last two days
...
check("paused: a daily maintenance Simulate is due after the run time",
      A._daily_maintenance_scan_due(dict(CFG, DAILY_RUN_TIME="00:01")) is True)
_seed(_run_epoch + 60)     # already scanned since today's run time
check("paused: not due once a scan ran after the run time",
      A._daily_maintenance_scan_due(dict(CFG, DAILY_RUN_TIME="00:01")) is False)
_seed(now - 3600)
check("paused: not due before today's run time",
      A._daily_maintenance_scan_due(dict(CFG, DAILY_RUN_TIME="23:59")) is False)
