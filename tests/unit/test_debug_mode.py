...
the scheduler isn't deleting (and blocks Automatic Cleanup), and it turns the dashboard's Cleanup button
into a yellow "Debug Cleanup" that fires the `debug_cleanup` engine mode. Guards
here:
  • config validation — DEBUG_MODE and Automatic Cleanup are mutually exclusive
...
# ── Config validation: Debug mode ⇔ NOT Automatic Cleanup ─────────────────────────────────
def _debug_issue(saved):
    return any(i.get("key") == "DEBUG_MODE" for i in A._config_file_issues(saved))

check("Debug mode + Automatic Cleanup is rejected",
...
check("Automatic Cleanup without Debug mode is fine",
